...
public class ViewDistributionFrame extends JFrame {
    ...
    public void updateDistribution(ReportSnapshot rep){
    	SwingUtilities.invokeLater(() -> {
    		panel.updateDistribution(rep);
    	});
    }
        
    public static class DistributionPanel extends JPanel {
        private volatile ReportSnapshot report;
        private int w;
        private int h;
        ...
        public void paint(Graphics g){
            ...
            final ReportSnapshot rep = report;
            
            if (rep != null) {
                int[] bands = rep.getBands();
	    		int max = 0;
	    		...
	    		if (max > 0) {
		    		...
		    		int nLocPerBand = rep.getMaxLoc()/(bands.length - 1);
		    		int a = 0;
		    		int b = nLocPerBand;
